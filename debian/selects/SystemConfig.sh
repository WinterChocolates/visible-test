...
command -v ufw &>/dev/null || $DEUB_PKG_MANAGER install ufw -y
...
show_firewall_status() {
    ...
    if command -v ufw &> /dev/null; then
        echo "=== UFW 状态 ==="
        ufw enable
        ufw status verbose || echo "UFW 未运行"
        echo ""
        echo "=== 规则列表 ==="
        ufw status numbered || echo "无规则"
    else
        echo "UFW 未安装"
    fi
}
    ...
    while true; do
        ...
        case $PORT_OPTION in
            ...
            3)
                ...
                whiptail --title "开放端口列表" --msgbox "$STATUS" 20 70
                ;;
            4)
                break
                ;;
...
2)
            ...
            8)
                ...
                ;;
            9)
                if whiptail --yesno "确定要启用防火墙吗？" 8 50; then
                    ufw enable
                    checkCommand "启用防火墙" && whiptail --msgbox "防火墙已启用" 8 40
                fi
                ;;
